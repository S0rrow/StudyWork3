...
public class SuperAccountInfoFrame extends JFrame{
	...
	SuperAccountInfoFrame(String username){
		...
		edit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				connection.close();
				new SuperAccountEditFrame(username, username);
				dispose();
			}
		});
		...
		insert.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(dialogResult==JOptionPane.YES_OPTION) {
					connection.close();
					new SuperAccountRegistrationFrame(username);
					dispose();
				}
			}
		});
	...
	private void synchronize(JPanel mainContainer, String superAccount) {
		...
		for(int i = 0; i < numRow; i++) {
			...
			edit.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					connection.close();
					new SuperAccountEditFrame(username.getText(), superAccount);
					dispose();
				}
			});
			JButton drop = new JButton("delete");
			drop.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if(dialogResult==JOptionPane.YES_OPTION) {
						...
						//System.out.println("account deleted from database!");
						connection.close();
						new SuperAccountInfoFrame(superAccount);
						dispose();
					}
				}
			});
